...
export class DeliveryService extends BaseService<DeliveryWithDetails, CreateDeliveryDto, UpdateDeliveryDto> {
  ...
  async create(data: CreateDeliveryDto): Promise<DeliveryWithDetails> {
    ...
    // Create delivery with calculated estimated time
    ...
    await this.eventBus.emit(new DeliveryCreatedEvent(
      delivery.id,
      delivery.clientId,
  ...
  async acceptDelivery(deliveryId: string, deliveryPersonId: string): Promise<DeliveryWithDetails> {
    ...
    // Emit delivery accepted event
    await this.eventBus.emit(new DeliveryAcceptedEvent(
      delivery.id,
      deliveryPersonId,
      delivery.clientId
    ));
  ...
  async updateStatus(
    ...
    await this.eventBus.emit(new DeliveryStatusChangedEvent(
      delivery.id,
      oldDelivery.status,
      delivery.status,
      changedBy
  ...
  async getDeliveriesByClient(clientId: string, page = 1, limit = 10): Promise<{
  ...
  async getDeliveriesByDeliveryPerson(deliveryPersonId: string, page = 1, limit = 10): Promise<{
    deliveries: DeliveryWithDetails[];
    total: number;
  }> {
    const [deliveries, total] = await Promise.all([
  ...
  async getAvailableDeliveries(page = 1, limit = 10): Promise<{
    deliveries: DeliveryWithDetails[];
    total: number;
  }> {
    const [deliveries, total] = await Promise.all([
  ...
  async getDeliveriesByStatus(status: string, page = 1, limit = 10): Promise<{
    deliveries: DeliveryWithDetails[];
    total: number;
  }> {
    const [deliveries, total] = await Promise.all([
